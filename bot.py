...
async def execute_trade(signal, amount, app):
    try:
        ...
        trade_log.append({
            "time": now_str(), "title": signal["title"][:40],
            "outcome": signal["outcome_label"], "amount": amount,
            "price": order.get("price", signal["bayse_price"]),
            "status": status, "category": signal["category"],
        })
        if len(trade_log) > 50:
            trade_log.pop(0)
        icon = "⚽" if signal["category"] == "sports" else "📈"
        if signal["category"] == "sports":
            detail = f"Gap: {int(signal['gap']*100)}% (Real: {int(signal['real_prob']*100)}% | Bayse: {fmt_price(signal['bayse_price'])})"
        else:
...
async def sports_scan_job(app):
    ...
    try:
        ...
        for event in events:
            ...
            if signal:
                await execute_trade(signal, max(100.0, budget / 5), app)
                traded += 1
                if traded >= 5:
                    break
...
async def cmd_start(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        "🤖 *Bayse Auto-Trader*\n\n"
        "Auto-trades sports & crypto on Bayse.\n\n"
        "/autotrade — Toggle ON/OFF\n"
        "/status — Bot status\n"
...
async def cmd_autotrade(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    note  = "_Bot is now scanning and trading._" if autotrade_enabled else "_Trading stopped._"
    await update.message.reply_text(
        f"{icon} *Auto-trading: {state}*\n\n"
        f"⚽ Sports — 20% wallet, every 30 min\n"
        f"📈 Crypto — 10% wallet, every 5 min\n"
        f"📐 Min gap: {int(MIN_GAP*100)}% | {CURRENCY}\n\n{note}",
        parse_mode="Markdown",
    )
...
async def cmd_status(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        s_bud   = fmt_ngn(balance * SPORTS_ALLOC)
        c_bud   = fmt_ngn(balance * CRYPTO_ALLOC)
    except Exception:
        bal_str = s_bud = c_bud = "N/A"
    await update.message.reply_text(
        f"📊 *Bot Status*\n\nAuto-trading: {icon} *{state}*\n\n"
        f"💰 Balance: *{bal_str}*\n"
        f"⚽ Sports budget: *{s_bud}* (20%)\n"
        f"📈 Crypto budget: *{c_bud}* (10%)\n\n"
        f"⏱ Sports: every 30 min | Crypto: every 5 min\n"
        f"📐 Min gap: {int(MIN_GAP*100)}%\n"
        f"🔄 Trades this session: {len(trade_log)}",
        parse_mode="Markdown",
    )

# ... (other command functions remain the same)
async def cmd_balance(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        for a in assets:
            lines.append(f"• {a.get('currency')}: {float(a.get('available', 0)):,.2f}")
    ...
    try:
        ...
        for pos in positions[:10]:
            lines.append(
                ...
                f"  {pos.get('outcome','?')} | {pos.get('shares','?')} shares | "
                f"₦{float(pos.get('currentValue', 0)):,.0f}"
            )
        await update.message.reply_text("\n".join(lines), parse_mode="Markdown")
    except Exception as e:
        await update.message.reply_text(f"❌ {e}")
...
async def cmd_trades(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    for t in reversed(trade_log[-10:]):
        ...
        lines.append(
            f"{icon} *{t['title']}*\n"
            f"  {t['outcome']} | ₦{t['amount']:,.0f} | {fmt_price(t['price'])} | `{t['status']}` | {t['time']}"
        )
    await update.message.reply_text("\n".join(lines), parse_mode="Markdown")
...
async def cmd_markets(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("⏳ Fetching…")
    try:
        s = await client.list_events(category="sports", size=5)
        c = await client.list_events(category="crypto", size=5)
        events = s.get("events", []) + c.get("events", [])
        if not events:
            await update.message.reply_text("No open markets.")
            return
        keyboard = []
        for ev in events[:10]:
...
async def button_handler(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    await query.answer()
    if query.data.startswith("ev:"):
        event_id = query.data[3:]
        try:
...
async def main():
    ...
    # Start background scan loops
    asyncio.create_task(sports_loop(app))
    asyncio.create_task(crypto_loop(app))

    await notify(app,
        "🤖 *Bayse Bot is online!*\n\n"
        "Send /autotrade to start trading.\n"
